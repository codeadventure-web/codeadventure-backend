...
def test_submission_updates_progress_and_returns_next_url(api_client, user_alice, course_python, problem_python, language_python):
    # Setup lessons
    l1 = Lesson.objects.create(
        course=course_python, title="L1", order=1, type=LessonType.JUDGE, problem=problem_python
    )
    l2 = Lesson.objects.create(
        course=course_python, title="L2", order=2, type=LessonType.JUDGE, problem=problem_python
    )
    ...
    # Payload
    data = {
        "language_id": language_python.id,
        "code": "print(1+1)"
    }
...
@pytest.mark.django_db
def test_submission_failed_does_not_complete(api_client, user_alice, course_python, problem_python, language_python):
    l1 = Lesson.objects.create(
        course=course_python, title="L1", order=1, type=LessonType.JUDGE, problem=problem_python
    )
...
@pytest.mark.django_db
def test_quiz_submission_updates_progress(api_client, user_alice, course_python, quiz_python):
    # Setup Quiz
    ...
    # Setup Lesson
    l1 = Lesson.objects.create(
        course=course_python, title="Quiz Lesson", order=1, type=LessonType.QUIZ, quiz=quiz_python
    )
