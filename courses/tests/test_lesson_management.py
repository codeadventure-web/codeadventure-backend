...
def test_create_lesson_as_teacher(api_client, user_teacher, course_python, problem_python):
...
@pytest.mark.django_db
def test_switch_lesson_type_clears_other_field(api_client, user_teacher, lesson_decorators, course_python, quiz_python):
    api_client.force_authenticate(user=user_teacher)
    url = f"/api/v1/{course_python.slug}/{lesson_decorators.slug}/"
